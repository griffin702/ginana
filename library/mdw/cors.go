...
func CORS(allowOriginHosts []string) iris.Handler {
	config := &CORSConfig{
...
func newCORS(config *CORSConfig) iris.Handler {
...
func (cors *cors) applyCORS(c iris.Context) {
	...
	if !cors.validateOrigin(origin) {
		log.Infof("The request's Origin header `%s` does not match any of allowed origins.", origin)
...
func (cors *cors) handlePreflight(c iris.Context) {
...
func (cors *cors) handleNormal(c iris.Context) {
	header := c.ResponseWriter().Header()
